        ...
        for track in tracks:
            ...
            if line_crossed is not None:
                ...
                if line_crossed not in self.crossing_times[track_id]:
                    ...
                    print(f"Current crossings for pedestrian {track_id}: {len(self.crossing_times[track_id])}")
                    print(f"Crossing times: {self.crossing_times[track_id]}")
            ...
            cv2.putText(processed_frame, f"ID:{track_id}", (int(bbox[0]), int(bbox[1])-10),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
            cv2.circle(processed_frame, (mid_x, mid_y), 4, (0, 0, 255), -1)
            ...
            if track_id in self.crossing_times:
                ...
                # Show which lines have been crossed
                text_y = int(bbox[3])+40
                for lane in range(3):
                    crossed_in_lane = []
                    for line in range(3):
                        line_num = lane * 3 + line
                        if line_num in self.crossing_times[track_id]:
                            crossed_in_lane.append(str(line + 1))
                    if crossed_in_lane:
                        cv2.putText(processed_frame, f"Lane {lane+1}: {','.join(crossed_in_lane)}", 
                                  (int(bbox[0]), text_y),
                                  cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
                        text_y += 20
